...
public class Drivetrain extends SubsystemBase {
  ...
  // private final EncoderSim leftEncoderSim = new EncoderSim(leftFrontEncoder);
  private final RelativeEncoder rightFrontEncoder = rightFrontMotor.getEncoder();
  // private final EncoderSim rightEncoderSim = new EncoderSim(rightFrontEncoder);
  ...
  private final BasePigeonSimCollection gyroSim = mGyro.getSimCollection();
  private final DifferentialDrivePoseEstimator mOdometry;
  private final DifferentialDriveKinematics mKinematics;

  private final SimpleMotorFeedforward feedForward = new SimpleMotorFeedforward(DrivetrainConstants.KS_LIN,
      DrivetrainConstants.KV_LIN, DrivetrainConstants.KA_LIN);
  ...
  public Drivetrain(Vision vision) {
    ...
    drivetrainSim = new DifferentialDrivetrainSim(
        LinearSystemId.identifyDrivetrainSystem(DrivetrainConstants.KV_LIN, DrivetrainConstants.KA_LIN,
    ...
    mOdometry = new DifferentialDrivePoseEstimator(mKinematics, mGyro.getRotation2d(), getLeftDistance(),
        getRightDistance(), new Pose2d());
  ...
  public void simulationPeriodic() {
    drivetrainSim.setInputs(leftFrontMotor.get() * RobotController.getInputVoltage(),
  ...
  public void drive(Supplier<Double> leftY, Supplier<Double> rightX, Supplier<Double> rightY,
  ...
  private void updatePose() {
    ...
    Optional<EstimatedRobotPose> cameraPose = mVision
        .getEstimatedGlobalPose(getPose2d().plus(new Transform2d(new Translation2d(0, 0), new Rotation2d(Math.PI))));
    // if(cameraPose.isPresent()){
    // mOdometry.addVisionMeasurement(cameraPose.get().estimatedPose.toPose2d(),
  ...
  // Brandon. Why does this return a string? Is this old code? - It is used for
  // shuffleboard
